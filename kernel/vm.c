...
uint64
uvmdealloc_kernel(pagetable_t pagetable, uint64 oldsz, uint64 newsz) {
    if (newsz >= oldsz)
        return oldsz;
...
// returns 0 on success, -1 on failure.
int
uvmcopy_kernel(pagetable_t u_pagetable, pagetable_t k_pagetable, uint64 start_sz, uint64 end_sz) {
    pte_t *pte, *pte_to;
    uint64 pa, uva;
    uint flags;
    if(start_sz > end_sz)
        return -1;
    for (uva = start_sz; uva < end_sz; uva += PGSIZE) {
        if ((pte = walk(u_pagetable, uva, 0)) == 0)
            panic("uvmcopy_kernel: user pte should exist");
        if ((*pte & PTE_V) == 0)
            panic("uvmcopy_kernel: user page not present");
        if ((pte_to = walk(k_pagetable, uva, 1)) == 0)
            panic("uvmcopy_kernel: k_pagetable walk fails");
        pa = PTE2PA(*pte);
        flags = PTE_FLAGS(*pte);
        flags = flags & (~PTE_U);
//        if (mappages(k_pagetable, uva, PGSIZE, pa, flags) != 0) {
//            goto err;
//        }
...
//    err:
//    uvmunmap(k_pagetable, 0, uva / PGSIZE, 0);
//    return -1;
}
...
void
vm_print_depth(pagetable_t pagetable, int depth){
    if(depth == 4) //没有第4级的页表了，需要退出递归
        return ;
    for (int i = 0; i < 512; i++) {
        pte_t pte = pagetable[i];
        if (pte & PTE_V) {
            ...
            uint64 child = PTE2PA(pte);
            printf("%d: pte %p pa %p\n",i,pte,child);
            vm_print_depth((pagetable_t) child, depth+1);
        }
    }
}
...
vmprint(pagetable_t pagetable){
    printf("page table %p\n",pagetable);
    int depth = 1;
    vm_print_depth(pagetable, depth);
}
