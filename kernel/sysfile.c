...
#define LINK_DEPTH_THRESHOLD 10
...
create(char *path, short type, short major, short minor) {
    ...
    if ((ip = dirlookup(dp, name, 0)) != 0) {
        iunlockput(dp);
        ilock(ip);
        if (type == T_FILE && (ip->type == T_FILE || ip->type == T_DEVICE))//T_File类型或者T_SYMLINK类型都直接返回
            return ip;
...
uint64
sys_symlink(void) {
    char sym_path[MAXPATH], target[MAXPATH];
    struct inode *ip;

    if (argstr(0, target, MAXPATH) < 0 || argstr(1, sym_path, MAXPATH) < 0)
        return -1;

    /**创建symbol link 文件*/
    begin_op();
    ip = create(sym_path, T_SYMLINK, 0, 0);
    if (ip == 0) {
        end_op();
        return -1;
    }
    /**将目标文件的文件路径写到inode里面*/
//    ilock(ip); //create()函数里就持有了对ip的锁
